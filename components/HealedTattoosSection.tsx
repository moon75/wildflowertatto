...
const staticSlides = [healed1, healed2, healed3, healed4, healed5, healed6, healed7, healed8];

type Slide = { src: string; alt: string };

export default function HealedTattoosSection() {
  const { t } = useLocale();
  const [baseSlides, setBaseSlides] = useState<Slide[]>([]);
    ...
    sanityClient
      ...
      .then((data) => {
        if (data && data.length > 0) {
          ...
          setBaseSlides(staticSlides.map((img, i) => ({
            src: img.src,
            alt: `Healed tattoo ${i + 1}`,
          })));
        }
      });
  }, []);

  const slides: Slide[] = [...baseSlides, ...baseSlides];
  ...
  const scrollByOne = useCallback((direction: "prev" | "next") => {
    ...
    const offset = firstSlide.offsetWidth + 16;
    scroller.scrollBy({ left: direction === "next" ? offset : -offset, behavior: "smooth" });
  }, []);
  ...
  useEffect(() => {
    const interval = setInterval(() => scrollByOne("next"), 5000);
...
function Chevron({ direction }: { direction: "left" | "right" }) {
  return (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="w-5 h-5 md:w-6 md:h-6" aria-hidden>
